...
- 通话摄像头：预览画面、切换前后镜头、按键截帧识别，也可直接说“看看这个是什么”
- 按住说话（语音识别 STT/ASR）
- 自动朗读（TTS，失败可回退浏览器语音）
...
5. 点“摄像头”并允许权限后，可以切换前后镜头；点“识别”或说“看看这个是什么”即可分析当前画面。
6. 通话界面可随时静音、关闭摄像头或挂断；切换页面到后台时会自动挂断并释放麦克风和摄像头。
...
上游接口返回的原始内容不会原样回显给页面，只返回其中的 JSON 错误信息或一句提示，避免把内网地址的响应内容带出来。
